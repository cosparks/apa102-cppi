...
Apa102::Apa102(uint32_t numLeds) : _numLeds(numLeds), _x(numLeds), _y(1) {
	// default orientation
...
void Apa102::init(uint32_t spiChan, uint32_t baud, uint32_t spiFlags) {
	_endframeLength = _calculateEndframe(_numLeds);
	_spiBufferLength = 4 + (_numLeds * 4) + _endframeLength;
	_spiBuffer = new uint8_t[_spiBufferLength](); // () initializes array to 0x0
	
	_assignIndexGetterFromOption();
	_writeEndframe();
...
void Apa102::setPixel(const Pixel& pixel, uint32_t i) {
	if (i >= _numLeds) {
		std::string message = "Apa102 Error: led out of active range!  Led " + std::to_string(i) +
			" is outside of active led range " + std::to_string(_numLeds);
...
void Apa102::setNumLeds(uint32_t value) {
...
void Apa102::_assignIndexGetterFromOption() {
